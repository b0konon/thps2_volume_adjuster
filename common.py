...
# Wavefront MTL helper class
class WavefrontMtl():
  def __init__(self):
    self._mtl = io.StringIO()

  def NewMaterial(self, name):
    self._mtl.write("newmtl %s\n" % name)

  def IlluminationMode(self, mode):
    self._mtl.write("illum %d\n" % mode)

  def DiffuseMap(self, map_Kd, scale = (1.0, 1.0, 1.0)):
    self._mtl.write("map_Kd -s %f %f %f %s\n" % (scale[0], scale[1], scale[2], map_Kd))

  def DissolveMap(self, map_d, scale = (1.0, 1.0, 1.0)):
    self._mtl.write("map_d -s %f %f %f %s\n" % (scale[0], scale[1], scale[2], map_d))

  def Save(self, path):
    with open(path, "w") as f:
      f.write(self._mtl.getvalue())
...
def write_string(f, s, length):
    b = s.encode('ascii')
    if len(b) > length:
        raise ValueError(f"String '{s}' is longer than allowed length {length}")
    # Pad with null bytes
    b = b + b'\0' * (length - len(b))
    f.write(b)
